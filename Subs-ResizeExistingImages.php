...
function ResizeMain($file)
{
	...
	// Determine the new width/height and make sure the aspect ration is retained
	require_once($sourcedir . '/Subs-AutoRotation.php');
	AutoRotation_Aspect($width, $height);

	// Set preferred format to JPEG if reformatting or image is BMP
	...
	{
		// Save the original file and set the timestamp of the new file to be the same as that of the original file
		$timestamp = filemtime($filename);
		$reiFilename = $attsPreREI . '/' . $file['id_attach'] . '_' . $file['file_hash'];
		rename($filename, $reiFilename);
		...
		error_log(print_r("resizeImageFile FAILED for" . $filename . " - file may contain invalid image information.", true));
}
